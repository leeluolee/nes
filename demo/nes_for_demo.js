!function(win, doc){
  // 为了可视化parse，find过程的修改后的nes.js

  var symbols = ""
  var index = 1
  var showData = function(parseData){
    var ul = nes.one("#data >ul"),
      result = ""

    for(var i =0;i<parseData.length;i++){
      result += "<li>" +parseData[i]+"</li>"
    }
    ul.innerHTML = result
  }
  var showSymbols = function(str,info,callback,step){
    index += step||1
    setTimeout(function(){
      nes.one("#rule").innerHTML = str
      nes.one("#info").innerHTML = info || ""
      callback && callback()
    },index*1000)
  }
  var reduce = function(str,parseData){
    var right = str.length
    symbols = symbols.slice(right)
    showSymbols(symbols, "匹配"+str+"成功",showData.bind(this,parseData),2)
  }
  ...
  var parse = function(sl){
    showSymbols(sl,"输入选择器")   
    var selector = clean(sl),parsed

    showSymbols(selector,"除去多余空格",null,3)   
    symbols = selector
  ...
  var 
    ...
    getTargets = function(lastPiece, context){
      ...
      var tmp = filterDatum(results, lastPiece, ignored)
      return tmp
    }
  // API 3 : find (private)
  // -------------
  ...
  var one = function(sl, context){
    ...
    try{
      ...
      // 这里不能在匹配第一个之后停止，因为还需要document sorter
